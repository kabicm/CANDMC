...
def createBashPreface(P, algorithm):
    ...
    return '#!/bin/bash -l \n\
#SBATCH --job-name=candmc-strong-%s-p%d \n\
#SBATCH --time=00:30:00 \n\
#SBATCH --nodes=%d \n\
#SBATCH --output=%s/-strong-%s-p%d-%s.txt \n\
...
def generateLaunchFile(P, N, B, r, algorithm, pivot):
    for p in P:
        filename = path_to_launch + 'launch_strong_%s_%d.sh' %(algorithm, p[0])
            ...
            for n in N:
                for b in B:
                    for pz in p[1]:
                        ...
                        pre = 'export SCOREP_EXPERIMENT_DIRECTORY=benchmarks/scorep/strong_%s_%d_%d_32x512_%s \n' %(algorithm, n, p[0], pz) 
